...
sub downloadFile {
	...
	while ($returnCode != 0 && $download_attempts < 10) {
		...
		if ('.txt' eq substr $filename, -length('.txt')) {
			$output = qx{_BPXK_AUTOCVT=ON _ENCODE_FILE_NEW=ISO8859-1 curl $curlOpts -k -o $filename $url 2>&1};
		} elsif ('.jar' eq substr $filename, -length('.jar')) {
			$output = qx{_BPXK_AUTOCVT=ON _ENCODE_FILE_NEW=BINARY curl $curlOpts -k -o $filename $url 2>&1};
		} else {
			$output = qx{_BPXK_AUTOCVT=ON _ENCODE_FILE_NEW=UNTAGGED curl $curlOpts -k -o $filename $url 2>&1};
		}
		$returnCode = $?;
